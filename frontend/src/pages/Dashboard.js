...
import { getPatients, getAlerts, getDoctors, getDashboardStats, getMyNotifications, markAllNotificationsRead, markNotificationRead, getVitals, getVitalsSourceConfig, getAllAIDiagnostics } from '../api';
...
export default function Dashboard() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      // Fetch AI diagnostics for the dashboard
      getAllAIDiagnostics().then(res => {
        const byPatient = {};
        (res.data || []).forEach(d => { byPatient[d.patient_id] = d; });
        setAiDiagnostics(byPatient);
      }).catch(() => {});
    ...
    <div style={{ animation: 'reveal 0.5s ease-out' }}>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card" style={{ borderLeftColor: '#a855f7' }}>
          <div className="label">🧠 AI Flagged</div>
          <div className="value" style={{ color: Object.values(aiDiagnostics).filter(d => d.classification !== 'AI_STABLE').length > 0 ? '#a855f7' : 'inherit' }}>
            {Object.values(aiDiagnostics).filter(d => d.classification !== 'AI_STABLE').length}
          </div>
        </div>
      </div>
        ...
        {loading ? <div className="spinner" /> : (
          <div style={{ overflowX: 'auto' }}>
            <table>
              <thead>
                <tr>
                  <th>Patient</th><th>Room</th><th>Doctor</th><th>Heart Rate</th><th>SpO₂</th><th>Temp</th><th>Status</th><th>AI State</th>
                </tr>
              </thead>
              <tbody>
                {patients.map(p => {
                  ...
                  return (
                    <tr key={p.patient_id}>
                      ...
                      <td>
                        {aiDiagnostics[p.patient_id] ? (
                          <span style={{
                            padding: '2px 8px', borderRadius: 4, fontSize: 9, fontWeight: 800,
                            background: aiDiagnostics[p.patient_id].classification === 'AI_STABLE' ? 'rgba(52,211,153,0.1)' :
                                        aiDiagnostics[p.patient_id].classification === 'AI_CRITICAL' ? 'rgba(244,63,94,0.15)' : 'rgba(251,191,36,0.1)',
                            color: aiDiagnostics[p.patient_id].classification === 'AI_STABLE' ? '#34d399' :
                                   aiDiagnostics[p.patient_id].classification === 'AI_CRITICAL' ? '#f43f5e' : '#fbbf24',
                          }}>
                            🧠 {aiDiagnostics[p.patient_id].classification.replace('AI_', '')}
                          </span>
                        ) : (
