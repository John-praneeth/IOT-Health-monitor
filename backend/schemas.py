...
class DoctorSelfRegister(ProjectBaseModel):
    ...
    password: str = Field(..., min_length=8, max_length=200, json_schema_extra={'str_strip_whitespace': False})
    name: str = Field(..., min_length=1, max_length=100)
...
class LoginRequest(ProjectBaseModel):
    ...
    password: str = Field(..., min_length=1, max_length=200, json_schema_extra={'str_strip_whitespace': False})


class ResetPasswordRequest(ProjectBaseModel):
    username: str = Field(..., min_length=1, max_length=100)
    new_password: str = Field(..., min_length=8, max_length=200, json_schema_extra={'str_strip_whitespace': False})
...
class ForgotPasswordConfirmRequest(ProjectBaseModel):
    username: str = Field(..., min_length=1, max_length=100)
    verification_code: str = Field(..., min_length=4, max_length=20, json_schema_extra={'str_strip_whitespace': False})
    new_password: str = Field(..., min_length=8, max_length=200, json_schema_extra={'str_strip_whitespace': False})
...
class MultimodalUpload(ProjectBaseModel):
    """Upload multimodal clinical data for AI analysis."""
    clinical_text: Optional[str] = Field(None, max_length=5000, description="Clinical notes or observations")
    image_b64: Optional[str] = Field(None, description="Base64-encoded medical image")
    audio_b64: Optional[str] = Field(None, description="Base64-encoded audio waveform")
    document_text: Optional[str] = Field(None, max_length=10000, description="Lab report or clinical document text")


class AIDiagnosticOut(ProjectBaseModel):
    """Full AI diagnostic report for a patient."""
    patient_id: int
    ...
    modality_embeddings: Optional[dict] = None
...
class EmbeddingSpaceOut(ProjectBaseModel):
    """Embedding space visualization data."""
    centroids: dict
    patients: list
    dimensions: int = 16
    projection_axes: List[int] = [0, 1]


class ClinicalDataCreate(ProjectBaseModel):
    modality: str = Field(..., pattern=r"^(text|image|audio|document)$")
    content: str = Field(..., min_length=1)
    filename: Optional[str] = Field(None, max_length=200)


class ClinicalDataOut(ProjectBaseModel):
    data_id: int
    patient_id: int
    modality: str
