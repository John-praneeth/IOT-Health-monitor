from sqlalchemy import Column, Integer, String, Float, Boolean, ForeignKey, TIMESTAMP, Text, Index, func, CheckConstraint, UniqueConstraint
...
# ═══════════════════════════════════════════════════════════════════════════════
#  v6.0 — MULTIMODAL AI TABLES
# ═══════════════════════════════════════════════════════════════════════════════

class ClinicalData(Base):
    """Stores multimodal clinical inputs for the AI embedding pipeline."""
    __tablename__ = "clinical_data"

    data_id      = Column(Integer, primary_key=True, index=True)
    patient_id   = Column(Integer, ForeignKey("patients.patient_id"), nullable=False, index=True)
    modality     = Column(String(20), nullable=False)    # 'text', 'image', 'audio', 'document'
    content      = Column(Text, nullable=False)           # Raw text or base64-encoded data
    filename     = Column(String(200), nullable=True)     # Original filename (for images/audio)
    ai_embedding = Column(Text, nullable=True)            # JSON-serialised embedding vector
    created_at   = Column(TIMESTAMP, server_default=func.now(), index=True)

    patient = relationship("Patient")

    __table_args__ = (
        Index("idx_clinical_data_patient_modality", "patient_id", "modality"),
        CheckConstraint(
            "modality IN ('text','image','audio','document')",
            name="ck_clinical_data_modality"
        ),
    )
