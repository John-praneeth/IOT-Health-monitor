...
# ═══════════════════════════════════════════════════════════════════════════════
#  AI / MULTIMODAL DIAGNOSTICS ENDPOINTS
# ═══════════════════════════════════════════════════════════════════════════════

@app.get("/ai/diagnostics/{patient_id}", tags=["AI Diagnostics"])
def get_ai_diagnostics(
    patient_id: int,
    current_user: models.User = Depends(auth.require_auth),
    db: Session = Depends(get_db),
):
    """Full AI diagnostic report using the Multimodal Unified Embedding Space."""
    ...
    try:
        result = ai_diagnostics.get_patient_diagnostics_from_db(patient_id, db)
        result["patient_name"] = p.name
        result["room_number"] = p.room_number
        return result
    except Exception as e:
        logger.error("AI diagnostics failed for patient %d: %s", patient_id, e)
        raise HTTPException(status_code=500, detail=f"AI diagnostics error: {str(e)}")


@app.get("/ai/diagnostics", tags=["AI Diagnostics"])
def get_all_ai_diagnostics(
    current_user: models.User = Depends(auth.require_auth),
    db: Session = Depends(get_db),
):
    """Batch AI diagnostics for all active patients."""
    import ai_diagnostics

    try:
        results = ai_diagnostics.get_all_patient_diagnostics(db)
        return results
    except Exception as e:
        logger.error("Batch AI diagnostics failed: %s", e)
        raise HTTPException(status_code=500, detail=f"AI diagnostics error: {str(e)}")


@app.post("/ai/upload/{patient_id}", tags=["AI Diagnostics"])
def upload_multimodal_data(
    patient_id: int,
    data: schemas.MultimodalUpload,
    current_user: models.User = Depends(auth.require_role("ADMIN", "DOCTOR", "NURSE")),
    db: Session = Depends(get_db),
):
    """
    Upload multimodal clinical data and run AI diagnosis.
    Accepts: clinical text, base64 image, base64 audio, document text.
    Stores data in clinical_data table and returns the AI diagnostic report.
    """
    import ai_diagnostics
    ...
    # Run diagnosis with all available data
    try:
        result = ai_diagnostics.get_patient_diagnostics_from_db(patient_id, db)
        result["patient_name"] = p.name
        result["room_number"] = p.room_number
        result["stored_modalities"] = stored
        return result
    except Exception as e:
        logger.error("AI upload diagnosis failed: %s", e)
        raise HTTPException(status_code=500, detail=f"AI diagnostics error: {str(e)}")


@app.get("/ai/embedding-space", tags=["AI Diagnostics"])
def get_embedding_space(
    current_user: models.User = Depends(auth.require_auth),
    db: Session = Depends(get_db),
):
    """Returns centroid positions and all patient embeddings for visualization."""
    import ai_diagnostics

    try:
        return ai_diagnostics.get_embedding_space_data(db)
    except Exception as e:
        logger.error("Embedding space data failed: %s", e)
        raise HTTPException(status_code=500, detail=f"Embedding space error: {str(e)}")


@app.get("/ai/clinical-data/{patient_id}", response_model=List[schemas.ClinicalDataOut], tags=["AI Diagnostics"])
def get_clinical_data(
    patient_id: int,
    current_user: models.User = Depends(auth.require_auth),
    db: Session = Depends(get_db),
):
    """Get stored clinical data for a patient."""
    p = crud.get_patient(db, patient_id)
    if not p:
        raise HTTPException(status_code=404, detail="Patient not found")
    check_patient_access(db, current_user, p)
    ...
    records = (
        ...
        .filter(models.ClinicalData.patient_id == patient_id)
