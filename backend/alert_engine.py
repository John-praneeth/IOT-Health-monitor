...
Integrates multimodal AI classification from the unified embedding space.
"""
...
def check_alerts(vital, db=None) -> list[str]:
    ...
    # ── 1b. Check AI Classification (Multimodal Unified Embedding Space) ──
    ai_label, ai_score = get_ai_classification(vital)
    if ai_label not in ["AI_STABLE", "AI_UNKNOWN", "AI_ERROR"]:
        raw_triggered.append(ai_label)
    ...
    if patient_id is not None:
        # Include AI labels in the sensitivity check
        all_possible_alerts = list(THRESHOLDS.keys())
        if ai_label not in ["AI_STABLE", "AI_UNKNOWN", "AI_ERROR"]:
            all_possible_alerts.append(ai_label)

        for alert_type in all_possible_alerts:
...
def get_ai_classification(vital):
    """
    Interface for ML-based classification using the Multimodal Unified
    Embedding Space engine.

    Encodes vitals through the multimodal pipeline (vitals modality only
    when called from the alert engine) and classifies in the unified ℝ¹⁶ space.
    """
    try:
        import ml_engine
        label, score = ml_engine.classify_health_state(
            vital.heart_rate, 
            vital.spo2, 
            vital.temperature
        )
