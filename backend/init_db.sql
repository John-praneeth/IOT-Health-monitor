...
-- 5. Multimodal AI Tables ──────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS clinical_data (
    data_id      SERIAL PRIMARY KEY,
    patient_id   INT NOT NULL REFERENCES patients(patient_id) ON DELETE CASCADE,
    modality     VARCHAR(20) NOT NULL CHECK (modality IN ('text','image','audio','document')),
...
CREATE INDEX IF NOT EXISTS idx_clinical_data_patient_modality ON clinical_data(patient_id, modality);
CREATE INDEX IF NOT EXISTS idx_clinical_data_created ON clinical_data(created_at);
